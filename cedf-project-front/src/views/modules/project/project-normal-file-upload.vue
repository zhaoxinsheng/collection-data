...
<script>
  export default {
    ...
    methods: {
      init (id) {
        this.url = this.$http.adornUrl(`/project/normal/import?token=${this.$cookie.get('token')}`)
        this.visible = true
      },
      uploadAction() {
        return this.$http.adornUrl(`/project/normal/import?token=${this.$cookie.get('token')}`);
      },
      // 上传之前
