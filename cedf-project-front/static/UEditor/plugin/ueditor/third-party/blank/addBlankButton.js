function UE_UI_BLANK (editor,uiName) {
	...
	me.addOutputRule(function (root) {
		...
		$(me.body).find('img.question-blank-space').each(function (index) {
			this.src = UEDITOR_CONFIG.UEDITOR_IMAGE_VISIT_URL + "plugin/ueditor/third-party/blank/" + ( index + 1 ) + ".png";
		...
		$.each(root.getNodesByTagName('img'), function (i, node) {
           if (hasClass(node, 'question-blank-space')) {
				...
				node.setAttr('src', UEDITOR_CONFIG.UEDITOR_IMAGE_VISIT_URL + "plugin/ueditor/third-party/blank/" + ( id + 1 ) + ".png");
